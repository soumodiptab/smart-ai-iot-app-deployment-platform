...
if __name__ == '__main__':
    ...
    client = MongoClient(MONGO_DB_URL)
    db = client.model_stat
    if (db.instance.count_documents({"modelId": "0"})) <= 0:
        db.instance.insert_one(
            {"modelId": "0", "class_0": 0, "class_1": 0, "class_2": 0, "class_3": 0, "class_4": 0})
    x = threading.Thread(target=script_function, args=())
    x.start()
    print('thread started')

    app.run(port=6015, debug=True, use_debugger=False, host='0.0.0.0',
            use_reloader=False, passthrough_errors=True, threaded=True)
