...
while True:
    ...
    if d[val] == 0:
        ...
        c1 += 1
        try:
            db.instance.update_one({"modelId": "0"}, {"$set": {"class_1": c1}})
        except Exception as e:
            log.error({'error': str(e)})
    elif d[val] == 2:
        c2 += 1
        try:
            db.instance.update_one({"modelId": "0"}, {"$set": {"class_2": c2}})
        except Exception as e:
            log.error({'error': str(e)})
    elif d[val] == 3:
        c3 += 1
        try:
            db.instance.update_one({"modelId": "0"}, {"$set": {"class_3": c3}})
        except Exception as e:
            log.error({'error': str(e)})
    elif d[val] == 4:
        c4 += 1
        try:
            db.instance.update_one({"modelId": "0"}, {"$set": {"class_4": c4}})
        except Exception as e:
            log.error({'error': str(e)})
    # j += 1
