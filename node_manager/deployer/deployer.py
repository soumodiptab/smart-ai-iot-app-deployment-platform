...
def startDeployment():
    ...
    print(ip)
    isDeployStart = True
    call_deployment_producer(app_id, app_instance_id,
                             isDeployStart, ip, isModel)
...
def call_deployment_producer(app_id, app_instance_id, isDeployStart, ip, is_model):
    ...
    if isDeployStart:
        ...
        deploy_producer.send(
            "deploy_" + ip, {"app_id": app_id, "app_instance_id": app_instance_id, "isModel": is_model})
    else:
        deploy_producer.send(
            "termiate_" + ip, {"app_id": app_id, "app_instance_id": app_instance_id, "isModel": is_model})
...
def get_deployment_node_to_stop(app_id, app_instance_id):
    address = getServiceAddress("node-manager")
    URL = "http://" + address + "/node-manager/app/getNode/" + \
...
def getServiceAddress(serviceId):
    URL = "http://" + cfg["initialiser"] + \
        "/initialiser/getService/" + serviceId
    r = requests.get(url=URL)
    data = r.json()
    ip = data["ip"]
    port = data["port"]
...
if __name__ == '__main__':
    ...
    client = HeartBeatClientForService('deployer')
    client.start()
